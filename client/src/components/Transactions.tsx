...
const TransactionCard = ({ addressTo, addressFrom, timestamp, message, keyword, amount, url }: any) => {
    const gifUrl: any = useFetch({ keyword });
    return (
        <div
    ...
    return (
        <div className="flex w-full justify-center items-center 2xl:px-20 gradient-bg-transactions">
            <div className="flex flex-col md:p-12 py-12 px-4">
                {currentAccount ? (
                    <h3 className="text-white text-3xl text-ceneter my-2">Latest Transactions</h3>
                ) : (
                    <h3 className="text-whtie text-3xl text-ceneter my-2">LConnect your account to see the latest transactions</h3>
                )}
                <div className="flex flex-wrap justify-center items-center mt-10">
                    {/* @ts-ignore */}
                    {transactions.reverse().map((transaction, i): any => (
                        <TransactionCard kley={i} {...transaction} />
                    ))}
                </div>
            </div>
        </div>
